...
# Railpack — required by the API pipeline to analyse and build user apps
# Uses mise as the version manager
...
Takes effect on the next WSL restart.
...
> **Why `NODE_TLS_REJECT_UNAUTHORIZED=0`?** In dev, the API connects to Vault directly using `VAULT_TOKEN` rather than Nomad workload identity JWT. Vault uses a self-signed TLS certificate — Node.js rejects self-signed certs by default. This flag disables that check for the dev server only. In production, the API runs inside a Nomad alloc and uses JWT auth, bypassing this issue entirely.

> **Why `VAULT_TOKEN` and `NOMAD_TOKEN` in `.env.local`?** In production these come from Vault workload identity and the Nomad ACL system respectively — the API never needs them as plain env vars. In dev, the API runs on the host without Nomad, so it needs the root Vault token to read secrets directly and the Nomad bootstrap token to submit jobs. These values are dynamic — they change on Vault reinit or Nomad ACL reset — so the `.env.local` file must be regenerated by re-running the command above after any such event.
...
Turborepo starts both the API (`localhost:3001`) and web (`localhost:5173`) with hot reload. The Vite dev server proxies `/api/*` requests to `localhost:3001`, stripping the `/api` prefix before forwarding — matching exactly how Caddy routes in production.
...
Then regenerate `.env.local`, run `./dev.sh`, and `pnpm dev` as above.
...
# Then regenerate apps/api/.env.local with the new NOMAD_TOKEN
```
...
### railpack must be on PATH when the API starts

The API pipeline calls `railpack` as a subprocess. If `railpack` is not on `PATH` when the API dev server starts, all pipeline builds will fail with `ENOENT: spawn railpack`. Mise activates `railpack` in your shell via `.bashrc` — always source it before starting the dev server:
